...
export default function EditMasterListPage() {
  ...
  useEffect(() => {
    const loadMasterList = async () => {
      try {
        ...
        const docRef = doc(firestore, 'master_lists', listId);
        const docSnap = await getDoc(docRef);

        if (docSnap.exists()) {
          const data = docSnap.data() as MasterList;
          setName(data.name);
      ...
      <div className="flex items-center gap-4 mb-6">
        ...
        <div className="flex-1">
          <h1 className="text-2xl font-bold flex items-center gap-2">
            ...
            Editar Lista Maestra
          </h1>
          <p className="text-muted-foreground">
            Modifique la estructura de la base de datos
          </p>
        </div>
          ...
          <CardContent className="space-y-4">
            ...
            <div className="p-3 bg-amber-50 dark:bg-amber-900/20 rounded-md text-sm">
              <div className="flex items-start gap-2">
                ...
                <div>
                  <p className="font-medium text-amber-700 dark:text-amber-300">Precaución</p>
                  <p className="text-amber-600 dark:text-amber-400">
                    Eliminar campos existentes puede afectar los datos ya guardados.
                    Los registros existentes mantendrán sus datos pero el campo no será visible.
                  </p>
                </div>
              </div>
            </div>
